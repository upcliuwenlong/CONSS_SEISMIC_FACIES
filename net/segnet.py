...
def init_model(settings):
    ...
    MODEL_NAME = cfg_train["model"]
    model = SegNet(model_name=MODEL_NAME,in_channels=model_cfg["in_channels"],classes=model_cfg["classes"],encoder_name=model_cfg["encoder_name"],
                   decoder_channels=model_cfg[MODEL_NAME]["decoder_channels"],rep_channels=model_cfg["rep_channels"],
                   upsampling=model_cfg[MODEL_NAME]["upsampling"])
...
class SegNet(nn.Module):
    def __init__(self,model_name, in_channels, classes, decoder_channels, upsampling, rep_channels=64,encoder_name="resnet101"):
