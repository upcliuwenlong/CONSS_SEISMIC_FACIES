...
def main(cfg_path):
    ...
    logger.info(cfg)
    LR = cfg["train"]["lr"]
    model_s,model_t = init_model(settings)
    optimizer = torch.optim.Adam(params=[{"params":model_s.parameters()}], lr=LR)
    lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=4, gamma=0.1)
    sup_loader, semi_loader, valid_loader = build_loader(cfg)
    for epoch in range(cfg["train"]["max_epoch"]):
        train(cfg, model_s, model_t ,optimizer, lr_scheduler, sup_loader, semi_loader, epoch, settings)
        validate(cfg, model_s, valid_loader, epoch, settings)

def train(cfg, model_s, model_t, optimizer, lr_scheduler, sup_loader, semi_loader, epoch, settings):
    model_s.train()
    model_t.train()
    ...
    for i_train in range(len(sup_loader)):
        ...
        loss = sup_loss + st_loss + 10 * con_loss
...
if __name__ == "__main__":
    ...
    cfg_path = cfg_dir + "seam_cct.yaml"
    main(cfg_path)
