...
def eval(args):
    ...
    inline_test_list = list(range(labels_data.shape[2]))
    inline_train_list = [0,100,200,300,400,500,600]
    for inline in inline_train_list:
        inline_test_list.remove(inline)

    predication = np.load(PREDICATION_PATH)['prediction']
    labels_data = labels_data[:, :, inline_test_list]
    predication = predication[:,:,inline_test_list]
